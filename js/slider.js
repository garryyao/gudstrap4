// Originate from [bootstrap-slider](http://seiyria.com/bootstrap-slider/)
(function(factory) {
          ...
          if (typeof options === 'string') {
            ...
            for (var i = 0, len = this.length; i < len; i++) {
              ...
              if (!$.isFunction(instance[options]) ||
                  options.charAt(0) === '_') {
                logError("no such method '" + options + "' for " + namespace +
                    " instance");
                continue;
              }
  ...
  (function($) {
    ...
    function createNewSlider(element, options) {
      ...
      for (var i = 0; i < optionTypes.length; i++) {
        ...
        val = (typeof val !== 'undefined') ? val : getDataAttrib(this.element,
            optName);
      ...
      if (this.sliderElem) {
        ...
        if (Array.isArray(this.options.ticks_labels) &&
          ...
          for (i = 0; i < this.options.ticks_labels.length; i++) {
            ...
            var noTickPositionsSpecified = this.options.ticks_positions.length ===
                0;
      ...
      var isValidHandleType = availableHandleModifiers.indexOf(
              this.options.handle) !== -1;
      ...
      if (this.options.tooltip === 'hide') {
        ...
        this.getTooltipTitle = this._getTooltipTitle.bind(this);
      ...
      if($ && $.fn.tooltip) {
        // Initialize tooltips
        this.$tooltip = $(this.options.range ? this.trackSelection : this.handle1);
        this.$tooltip.attr('data-toggle', 'tooltip').tooltip({
          title: this.getTooltipTitle,
          template: '<div class="tooltip ef-tooltip" role="tooltip">' +
            '<div class="tooltip-arrow"></div>' +
            '<div class="tooltip-inner"></div>' +
          '</div>',
          trigger: 'manual',
          placement: this.tooltip_position,
        });
      }
    }
    ...
    Slider.prototype = {
      ...
      destroy: function() {
        // Hides and destroys the slider's tooltip
      ...
      _removeSliderEventHandlers: function() {
        ...
        if (this.showTooltip) {
          this.sliderElem.removeEventListener("mouseenter", this.showTooltip,
              false);
        }
        if (this.hideTooltip) {
          this.sliderElem.removeEventListener("mouseleave", this.hideTooltip,
              false);
        }
        this.sliderElem.removeEventListener("touchstart", this.mousedown,
            false);
      ...
      _getTooltipTitle: function() {
        var tooltipVal = this.options.range
            ? this._state.value
            : this._state.value[0];
        return this.options.formatter(tooltipVal);
      },
      _updateTooltip: function() {
        var it;
        var $tip;

        // Update tooltip title and re-position
        if (this.$tooltip) {
          it = this.$tooltip.data('bs.tooltip');
          $tip = $(it.getTipElement());
          it.setElementContent($tip.find('.tooltip-inner'), it.getTitle());
          if(it._tether){
            it._tether.position();
          }
        }
      ...
      _layout: function() {
        ...
        if (Array.isArray(this.options.ticks) &&
          ...
          var styleSize = this.options.orientation === 'vertical'
              ? 'height'
              : 'width';
          var styleMargin = this.options.orientation === 'vertical'
              ? 'marginTop'
              : 'marginLeft';
          ...
          if (this.tickLabelContainer) {
            ...
            if (this.options.ticks_positions.length === 0) {
              ...
              for (i = 0; i < this.tickLabelContainer.childNodes.length; i++) {
                if (this.tickLabelContainer.childNodes[i].offsetHeight >
                    extraMargin) {
                  extraMargin =
        ...
        if (this.options.orientation === 'vertical') {
          ...
          this.trackHigh.style.height =
              (100 - Math.min(positionPercentages[0], positionPercentages[1]) -
              Math.abs(positionPercentages[0] - positionPercentages[1])) + '%';
        } else {
      ...
      _keydown: function(handleIdx, ev) {
        ...
        if (this.options.natural_arrow_keys) {
          var ifVerticalAndNotReversed = (this.options.orientation ===
          'vertical' && !this.options.reversed);
          var ifHorizontalAndReversed = (this.options.orientation ===
          'horizontal' && this.options.reversed);
      ...
      _adjustPercentageForRangeSliders: function(percentage) {
        if (this.options.range) {
          ...
          if (this._state.dragged === 0 &&
              this._applyToFixedAndParseFloat(this._state.percentage[1],
                  precision) < percentageWithAdjustedPrecision) {
            this._state.percentage[0] = this._state.percentage[1];
            this._state.dragged = 1;
          } else if (this._state.dragged === 1 &&
              this._applyToFixedAndParseFloat(this._state.percentage[0],
                  precision) > percentageWithAdjustedPrecision) {
